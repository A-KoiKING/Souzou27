...
void init_pinMode(void)
{
    ...
    pin_mode[PIN18_RA1] = ICSPCLK;
    pin_mode[PIN19_RA0] = ICSPDAT;
    pin_mode[PIN20_VSS] = VSS;
}

void pinMode(const unsigned char pinno, const unsigned char mode)
{
    if(pinno > 20 ) return;
    if(pinno == PIN01_VDD ) return;
    if(pinno == PIN04_RA3 ) return;
    if(pinno == PIN18_RA1 ) return;
    if(pinno == PIN19_RA0 ) return;
    if(pinno == PIN20_VSS ) return;
